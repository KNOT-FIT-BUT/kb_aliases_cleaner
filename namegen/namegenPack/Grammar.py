...
class Terminal(object):
    ...
    def __init__(self, terminalType, attr=None, morph=True):
        """
        Vytvoření terminálu.
        ...
        if not any(a.type == self.Attribute.Type.PRIORITY for a in attr):
            ...
            attr = attr | {self.Attribute(self.Attribute.Type.PRIORITY, 0)}
    ...
    def tokenMatchWithoutCache(self, t):
        ...
        if (
            t.type == Token.Type.ANALYZE_UNKNOWN
            and self.type in self.UNKNOWN_ANALYZE_TERMINAL_MATCH
        ):
...
class Lex(object):
    ...
    def isTitle(cls, name, pos):
        ...
        if str(w) in cls.__TITLES or (
            str(w) in cls.__TITLES_PREFIXES and pos < len(name)
        ):
            ...
            if str(w) in cls.__TITLES_PREFIXES and pos < len(name):
                ...
                while str(actTitlePrefix) in cls.__TITLES_PREFIXES and lookAhead < len(
                    name
                ):
...
class AnalyzedToken(object):
    ...
    def token(self):
        """
        :return: Pro který máme tuto analýzu.
        :rtype: Token
        """
        return self._token
    ...
    def morphCategories(self) -> Set[MorphCategory]:
        """
        Získání morfologických kategorií, které na základě analýzy má dané slovo patřící k tokenu mít. Vybere jen ty
        hodnoty, které v nějaké z kategorii zpřesňují odhad , tedy pokud analýza určí, že dané slovo může mít pouze
        hodnoty v kategorii DegreeOfComparison 1 a 2, tak tyto hodnoty vrátí. Kdyby mohlo slovo nabývat všech hodnot,
        tak je vůbec nevrací.
        ...
        if self.matchingTerminal.type.isPOSType:
            ...
            for mCat, morphCategoryValues in morphsInfo.items():
                if mCat == MorphCategories.NOTE:
                    # Nechceme použít, jelikož se jedná o nepovinný atribut, který není v morfologické
                    # analýze uveden vždy.
                    # Není zde řeč o nepovinných attributech terminálů, které již mohly být vloženy výše.
                    # Zde mluvíme o tom, že některé skupiny v morfologické analýze mohou mít tento atribut
                    # zcela vynechaný.
                    continue
...
class Grammar(object):
    ...
    class ParsingTableSymbolRow(dict):
        """
        Reprezentuje řádek parsovací tabulky, který odpovídá symbolu. Chová se jako dict() s tím rozdílem,
        ...
        def __getitem__(self, key):
            ...
            vybere všechna pravidla (vrací množinu pravidel), která je možné aplikovat pro daný token (jeden token
            může odpovídat více terminálům).
    ...
    def _addDefValNon(
        ...
        :raise InvalidGrammarException:
        Pokud netermínál na pravé straně pravidla, se nevyskytuje nikde na levé straně nějakého pravidla.
        """
    ...
    def _eliminatingEpRules(self):
        ...
        for r in self._rules:
            if not r.rightSide == [self.EMPTY_STR]:  # pravidla A ->ε vynecháváme
                # vytvoříme všechny kombinace z pravidla, kde můžeme vynechat 0-x neterminálu,
                # které se derivují na empty
    ...
    def _removeUnaryRules(self):
        ...
        # Lecture 12: Cleaning upCFGs and Chomsky Nor-mal form,
        # CS 373: Theory of Computation ̃Sariel Har-Peled and Madhusudan Parthasarathy
    ...
    def _makeGroups(self):
        ...
        for r in self._rules:
            ...
            try:
                ...
                searchAccordingToLeftSide[r.leftSide] = {r}
    ...
    def _makeFollowSets(self):
        ...
        self._follow[self._startS] = {Terminal(Terminal.Type.EOF)}
