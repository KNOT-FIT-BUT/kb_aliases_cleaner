...
class Generator(ABC):
    ...
    @abstractmethod
    def __call__(
        self, nameMorphs: List[NameMorph]
    ) -> List[Tuple[Name, List[NameMorph]]]:
        """
        Volání generátoru.
...
class GenerateAbbreFormOfPrep(Generator):
    ...
    def __call__(
        ...
        if (
            len(nameMorphs[0].wordsMorphs) > 2
            and any(
                nameMorphs[0].forName.type == alloT for alloT in self.allowedNameTypes
            )
            and any(
                nameMorphs[0].wordsTypes[i][0] == WordTypeMark.PREPOSITION
                and len(  # jedná se o předložku
                    next(iter(nameMorphs[0].wordsMorphs[i]))[0]
                )
                > 1  # delší než jeden znak
                for i in range(1, len(nameMorphs[0].wordsMorphs) - 1)
            )
        ):  # hledáme jen uprostřed jména
            ...
            for nameM in nameMorphs:
                newWordsM = []
                newWordsTypes = []
                for i, (wordMorphs, wordMark) in enumerate(
                    zip(nameM.wordsMorphs, nameM.wordsTypes)
                ):

                    if (
                        i != 0
                        and i != len(nameM.wordsMorphs) - 1
                        and wordMark[0] == WordTypeMark.PREPOSITION
                        and len(next(iter(wordMorphs))[0]) > 1
                    ):
                        # máme předložku pro zkrácení
                        # vyrobíme zkratky (nad -> n.)
                        newWordMorphs = {
                            (wordM[0] + ".", wordRule) for wordM, wordRule in wordMorphs
                        }
                        newWordsM.append(newWordMorphs)
                        newWordsTypes.append(
                            (WordTypeMark.PREPOSITION_ABBREVIATION, wordMark[1])
                        )
