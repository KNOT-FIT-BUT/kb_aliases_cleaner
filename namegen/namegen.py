...
class ConfigManager(object):
    ...
    def __transformMorphoAnalyzer(self):
        ...
        result = {
            "PATH_TO": self.getAbsolutePath(
                self.configParser[self.sectionMorphoAnalyzer]["PATH_TO"]
            )
        }
    ...
    def __transformGrammar(self):
        ...
        result = {
            "TITLES": self._readTitles(
                self.getAbsolutePath(self.configParser[self.sectionGrammar]["TITLES"])
            ),
...
def main():
    ...
    try:
        ...
        if configAll[configManager.sectionGrammar]["TITLES"]:
            ...
            namegenPack.Grammar.Lex.setTitles(
                configAll[configManager.sectionGrammar]["TITLES"]
            )
        ...
        # načtení gramatik
        try:
            grammarMale = namegenPack.Grammar.Grammar(
                configAll[configManager.sectionDataFiles]["GRAMMAR_MALE"],
        ...
        try:
            grammarFemale = namegenPack.Grammar.Grammar(
                configAll[configManager.sectionDataFiles]["GRAMMAR_FEMALE"],
                configAll[configManager.sectionGrammar]["TIMEOUT"],
            )
        except Errors.ExceptionMessageCode as e:
            raise Errors.ExceptionMessageCode(
        ...
        try:
            grammarEvents = namegenPack.Grammar.Grammar(
                configAll[configManager.sectionDataFiles]["GRAMMAR_EVENTS"],
                configAll[configManager.sectionGrammar]["TIMEOUT"],
            )
        ...
        namesFilter = NamesFilter(
            configAll[configManager.sectionFilters]["LANGUAGES"],
        ...
        grammarsForTypeGuesser = {
            Name.Type.PersonGender.FEMALE: grammarFemale,
        ...
        print(
            "\t\t\t Počet analýz:", grammarFemale.grammarNumOfAnalyzes, file=sys.stderr
        )
        ...
        print(
            "\t\t\t Průměrný čas strávený nad jednou syntaktickou analýzou:",
            grammarLocations.grammarEllapsedTime / grammarLocations.grammarNumOfAnalyzes
        ...
        print(
            "\t\t\t Počet analýz:", grammarEvents.grammarNumOfAnalyzes, file=sys.stderr
        )
